...
def create_similarity_matrix(matrix,imp_word_in_sentence):
    for i in range(len(imp_word_in_sentence)):
        for j in range(len(imp_word_in_sentence)):
            if i == j:
                continue
        matrix[i][j]= sentence_similarity(imp_word_in_sentence[i],imp_word_in_sentence[j])
    return matrix
...
def create_summary(file_name,per):
    ...
    similarity_matrix = create_similarity_matrix(matrix,imp_word_in_sentence)
